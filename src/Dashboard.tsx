...
const Dashboard: React.FC<DashboardProps> = ({ historial }) => {
  // 1. LÓGICA DINÁMICA: Agrupamos por cualquier calidad que devuelva la IA
  const qualityMap = historial.reduce((acc: any, item: any) => {
    const q = item?.calidad || "Desconocida";
    // Normalizamos para agrupar (ej: "Premium" y "premium" son lo mismo)
  ...
  // 2. COLORES INTELIGENTES: Asignamos colores fijos a conocidos y aleatorios a nuevos
  ...
  // Porcentaje de Alta Gama (Ultra + Premium)
  ...
  return (
    <section className="bg-white p-6 rounded-[2.5rem] shadow-xl border-2 border-orange-50 animate-in zoom-in-95 duration-500 text-left">
        ...
        {/* GRID DINÁMICO: Se ajusta según la cantidad de categorías encontradas */}
