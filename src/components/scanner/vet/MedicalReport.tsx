...
const MedicalReport = ({ data }: { data: any }) => {
  // Si no hay datos o el objeto está vacío, no mostramos nada para evitar errores
  if (!data || Object.keys(data).length === 0) return null;

  // Mapeo seguro de colores de urgencia
  ...
  return (
    <div className="space-y-6 text-left animate-in fade-in duration-700 w-full">
      ...
      <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 flex gap-4">
        ...
        <div className="flex-1 min-w-0"> {/* min-w-0 ayuda a que el texto no desborde en móbiles pequeños */}
          <p className="text-[10px] font-black uppercase text-slate-400 mb-1 tracking-widest leading-none">Hallazgos</p>
      ...
      {/* 2. Tarjeta de Urgencia Dinámica */}
      ...
      <div className="space-y-3 w-full">
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Pasos a seguir:</p>
        {data.pasos_a_seguir && Array.isArray(data.pasos_a_seguir) && data.pasos_a_seguir.length > 0 ? (
          data.pasos_a_seguir.map((paso: string, i: number) => (
            <div key={i} className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center gap-3 shadow-sm animate-in slide-in-from-right-3 w-full" style={{ animationDelay: `${i * 100}ms` }}>
          ...
          <p className="text-xs italic text-slate-400 ml-4">No se indicaron pasos específicos.</p>
        )}
      </div>

      {/* 4. Conclusión Final (Resumen) */}
      <div className="bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group w-full">
        <Sparkles className="absolute -right-2 -top-2 text-white/10 group-hover:rotate-12 transition-transform duration-700" size={80} />
        <div className="relative z-10">
