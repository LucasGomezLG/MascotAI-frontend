...
import {ArrowDownNarrowWide, ExternalLink, ShoppingBag, X} from 'lucide-react';
import type {OfertaPrecioDTO} from '@/types/api.types.ts';

interface PriceComparisonModalProps {
  isOpen: boolean;
  onClose: () => void;
  busquedaResult: OfertaPrecioDTO[];
}

const PriceComparisonModal = ({ isOpen, onClose, busquedaResult }: PriceComparisonModalProps) => {
  const [sortedItems, setSortedItems] = useState<OfertaPrecioDTO[]>([]);

  useEffect(() => {
    setSortedItems(busquedaResult);
  }, [busquedaResult]);
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 relative animate-in zoom-in-95 text-left shadow-2xl">
        ...
        <div className="space-y-3 mb-8 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
          {sortedItems.length > 0 ? sortedItems.map((o, i) => (
            <div key={i} className="bg-slate-50 p-5 rounded-[1.5rem] border border-slate-100 flex flex-col gap-2 relative group overflow-hidden">
              ...
              <div className="flex justify-between items-end">
                <span className="text-2xl font-black text-orange-600">{formatPrice(parseFloat(o.precio))}</span>
