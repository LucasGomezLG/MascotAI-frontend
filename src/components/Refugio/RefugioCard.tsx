...
import { MapPin, Trash2, Globe, Wallet, Copy, Check, ChevronLeft, ChevronRight, MessageSquare, ExternalLink } from 'lucide-react';
...
const RefugioCard = ({ refugio, currentUser, onDelete }: RefugioCardProps) => {
  ...
  const [copiedAlias, setCopiedAlias] = useState(false); // ✅ Estado para el Alias
  const [copiedSocial, setCopiedSocial] = useState(false); // ✅ Estado para la Red Social
  ...
  const handleCopySocial = () => {
    ...
    showToast('Contacto copiado con éxito');
    ...
    <div className="relative min-w-[280px] max-w-[280px] bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-slate-100 flex flex-col group animate-in fade-in">
      ...
      <div className="relative h-44 bg-slate-100 group/img">
        ...
        <div className="absolute bottom-3 left-3 px-3 py-1 bg-violet-600 text-white rounded-full shadow-lg z-10">
          <span className="text-[8px] font-black uppercase tracking-widest">Refugio</span>
        </div>
      </div>
      ...
      <div className="p-5 space-y-4 text-left">
        ...
        {/* BOTONES DE ACCIÓN (COPIADO INTELIGENTE) */}
